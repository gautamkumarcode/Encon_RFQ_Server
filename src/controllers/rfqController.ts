...
import { mirrorAttachmentToDrive, ensureEnquiryDriveFolder } from '../services/gdriveService';
...
function sanitizeOfferNo(offerNo: string | undefined, rfqId: string | undefined): string {
  if (!offerNo) return '';
  const trimmed = offerNo.trim();
  if (!trimmed) return '';
  if (rfqId && (trimmed === rfqId || trimmed === rfqId.replace('/RFQ/', '/OFR/'))) {
    return '';
  }
  return trimmed;
}
      ...
      if (buffer.readUInt32LE(offset) === 0x04034b50) {
        ...
        if (fileName.includes('word/') || fileName.includes('document.xml')) {
          ...
          try {
            ...
            if (matches) {
              text += ' ' + matches.map((t) => t.replace(/<[^>]+>/g, '').trim()).join(' ');
            }
          } catch (e) {}
        }
...
export const extractOfferDetailsFromDoc = async (
  ...
  try {
    if (lowerFn.endsWith('.pdf') || !lowerFn.includes('.')) {
      ...
      const pdfParseFunc = typeof pdfParse === 'function' ? pdfParse : require('pdf-parse');
      ...
      if (!text) {
        ...
        if (textMatches) {
          text = textMatches.map((t) => t.replace(/<[^>]+>/g, '').trim()).join(' ');
        } else {
          text = utf8Text.replace(/[^\x20-\x7E\n\r]/g, ' ');
        }
      }
    } else if (lowerFn.endsWith('.xlsx') || lowerFn.endsWith('.xls') || lowerFn.endsWith('.csv')) {
  ...
  if (!text.trim()) {
    return { enquiryNo: '', offerNo: '', offerDate: '', clientName: '', projectName: '' };
  }
...
function cleanRefToken(raw: string): string {
  ...
  cleaned = cleaned.split(/\s+(?:DT|Date|Dated|Client|Company|Subject|Sub|Dear|Kind|PP|Tel|Mobile|Email|TABLE)\b/i)[0].trim();
  ...
  for (const pattern of patterns) {
    ...
    if (match && match[1]) {
      ...
      if (cand.length > 3 && !['number', 'details', 'date'].includes(cand.toLowerCase())) {
        offerNo = cand;
        break;
      }
    }
  ...
  if (!offerNo) {
    for (const l of lines) {
      const match = l.match(/(?:Enquiry|Enq(?:uiry)?|ENQ|Offer|Quotation|Quote|Ref)\s*(?:No|Number|#)?\.?\s*[:\-]?\s*(.+)$/i);
      if (!match) continue;
  ...
  return { enquiryNo, offerNo, offerDate, clientName, projectName, rawText: text.substring(0, 500) };
};
...
const CLOSED_STATUSES = new Set(['closed', 'won', 'lost', 'cancelled', 'rejected', 'po received', 'regret']);
const STANDARD_TAT_DAYS = 30;

const FULL_ACCESS_ROLES = new Set(['ADMIN', 'CO', 'GM', 'PRODUCTION_HEAD', 'SALES_MARKETING']);
const CAN_REVIEW_ROLES = new Set(['ADMIN', 'CO', 'GM', 'PRODUCTION_HEAD']);
const CAN_EDIT_ROLES = new Set(['ADMIN', 'CO', 'GM', 'PRODUCTION_HEAD', 'SALES_MARKETING']);

export function canSeeFullRfqList(user?: any): boolean {
  if (!user) return true;
  const roleName = (user.roleName || user.role?.name || user.role || '').toUpperCase();
  if (FULL_ACCESS_ROLES.has(roleName)) return true;
  if (Array.isArray(user.permissions) && (user.permissions.includes('RFQ:READ_ALL') || user.permissions.includes('USER_MGMT:MANAGE'))) return true;
  return false;
}
...
export function canReviewRfq(user?: any): boolean {
  ...
  if (Array.isArray(user.permissions) && user.permissions.includes('RFQ:REVIEW')) return true;
  return false;
}
...
export function canFinalApproveRfq(user?: any): boolean {
  ...
  if (Array.isArray(user.permissions) && user.permissions.includes('RFQ:FINAL_APPROVE')) return true;
  return false;
}
...
export function canEditRfq(user?: any): boolean {
  ...
  if (Array.isArray(user.permissions) && user.permissions.includes('RFQ:WRITE')) return true;
  return false;
}
...
export function isAdmin(user: any): boolean {
  if (!user) return true;
  const roleName = (user.roleName || user.role?.name || user.role || '').toUpperCase();
  if (roleName === 'ADMIN' || roleName === 'CO' || roleName === 'GM' || roleName === 'PRODUCTION_HEAD') return true;
  if (Array.isArray(user.permissions) && (user.permissions.includes('RFQ:DELETE') || user.permissions.includes('USER_MGMT:MANAGE'))) return true;
  return false;
}
...
export function canChangeStatus(user: any): boolean {
  if (!user) return true;
  const roleName = (user.roleName || user.role?.name || user.role || '').toUpperCase();
  if (roleName === 'ADMIN' || roleName === 'CO' || roleName === 'GM' || roleName === 'PRODUCTION_HEAD' || roleName === 'SALES_MARKETING') return true;
  if (Array.isArray(user.permissions) && (user.permissions.includes('RFQ:CHANGE_STATUS') || user.permissions.includes('RFQ:WRITE'))) return true;
  return false;
}
...
function calculateDaysOpen(receivedOn?: string, status?: string): number | null {
...
function calculateTentativeOfferDate(assignedDate?: string, dateReceived?: string, tatStr?: string): string | null {
...
function isFollowupDue(assignedDate?: string, dateReceived?: string, tatStr?: string, status?: string): boolean {
...
export const getEnquiries = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { status, assignedTo, search, due, overdue, mapped, tab, sort = '_id', dir = 'desc' } = req.query;
    ...
    let enriched = rawEnquiries.map((e) => {
      const daysOpen = calculateDaysOpen(e.receivedOn || e.dateReceived, e.status);
      const ageClass = calculateAgeClass(daysOpen);
      const tentativeOfferDate = calculateTentativeOfferDate(e.assignedDate, e.dateReceived, e.tat);
      const followupDue = isFollowupDue(e.assignedDate, e.dateReceived, e.tat, e.status);
    ...
    const pageItems = enriched.slice((clampedPage - 1) * limitNum, clampedPage * limitNum);
    ...
    const allRawEnquiries: any[] = await Enquiry.find(baseStatsWhere)
      .select('status assignedTo dateReceived receivedOn assignedDate tat offerNo rfqId')
      .lean();
    ...
    const stats = {
      ...
      incompleteCount: allEnriched.filter((e) => (e.status || '').toLowerCase() === 'incomplete').length,
      underReview: allEnriched.filter((e) => (e.status || '').toLowerCase() === 'under review' || (e.status || '').toLowerCase() === 'verified').length,
      offerSent: allEnriched.filter((e) => (e.status || '').toLowerCase() === 'offer sent').length,
      ...
      approvedCount: allEnriched.filter((e) => (e.status || '').toLowerCase() === 'approved').length,
...
export const getOfferMapping = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const all: any[] = await Enquiry.find(userScopeWhere).sort({ _id: -1 }).lean();
    const filtered: any[] = await Enquiry.find(where)
      .select('rfqId offerNo offerDate companyName itemDescription status')
      .sort({ _id: -1 })
      .lean();

    const quoted = all.filter((e) => Boolean(sanitizeOfferNo(e.offerNo, e.rfqId))).length;
...
export const getAnalyticsDashboard = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const enquiries: any[] = await Enquiry.find(userScopeWhere).sort({ _id: -1 }).lean();
    ...
    const avgOpenAge = activeDaysOpen.length > 0 ? Math.round(activeDaysOpen.reduce((a, b) => a + b, 0) / activeDaysOpen.length) : 0;

    const summary = {
      total: enquiries.length,
      open: enriched.filter((e) => (e.status || '').toLowerCase() === 'open').length,
      underReview: enriched.filter((e) => (e.status || '').toLowerCase() === 'under review').length,
      offerSent: enriched.filter((e) => (e.status || '').toLowerCase() === 'offer sent').length,
    ...
    const assigneeMap = new Map<string, { active: number; overdue: number; ageSum: number }>();
    activeEnquiries.forEach((e) => {
      const name = e.assignedTo || 'Unassigned';
      const curr = assigneeMap.get(name) || { active: 0, overdue: 0, ageSum: 0 };
      curr.active += 1;
    ...
    const byStatus = Object.entries(statusCounts).map(([label, count]) => ({
      label,
      count,
      pct: enquiries.length > 0 ? Math.round((count / enquiries.length) * 100) : 0,
    }));
...
export const getEnquiryById = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const attachments: any[] = await Attachment.find({ enquiryId: id }).select('-data').lean();

    const userInfo = await getAuthenticatedUserInfo(req);
    const isFullAccess = canSeeFullRfqList(userInfo || req.user);
    if (!isFullAccess && userInfo) {
      const isAssigned =
        (enquiry.assignedTo && (enquiry.assignedTo === userInfo.name || enquiry.assignedTo === userInfo.email)) ||
        (enquiry.salesResponsibility && enquiry.salesResponsibility === userInfo.name) ||
    ...
    const daysOpen = calculateDaysOpen(enquiry.receivedOn || enquiry.dateReceived, enquiry.status);
    ...
    const enriched = {
      ...
      tentativeOfferDate: calculateTentativeOfferDate(enquiry.assignedDate, enquiry.dateReceived, enquiry.tat),
      followupDue: isFollowupDue(enquiry.assignedDate, enquiry.dateReceived, enquiry.tat, enquiry.status),
      isMappedToOffer: Boolean(cleanOfferNo),
    };
...
export const createEnquiry = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await logActivity({
      ...
      action: 'RFQ_CREATED',
      details: { enquiryId: enquiry._id.toString(), rfqId: enquiry.rfqId, company: enquiry.companyName },
    });

    if (enquiry.assignedTo) {
      createAssignmentNotification(enquiry, enquiry.assignedTo, req.user?.email || 'User').catch(() => {});
    }
    if (enquiry.technical && enquiry.technical !== enquiry.assignedTo) {
      createAssignmentNotification(enquiry, enquiry.technical, req.user?.email || 'User').catch(() => {});
    }

    const cleanData = { ...enquiry.toObject(), id: enquiry._id.toString(), emailBody: undefined };
...
async function createAssignmentNotification(enquiry: any, assigneeStr: string, assignerEmail: string) {
  if (!assigneeStr || !assigneeStr.trim()) return;

  try {
    const targetStr = assigneeStr.trim().toLowerCase();

    const allUsers: any[] = await User.find({ status: 'ACTIVE' }).populate('roleId').lean();
    ...
    let matchedUsers = allUsers.filter((u) => {
      ...
      return (
        ...
        (targetStr.includes('tech') && (uRole.includes('tech') || uRole.includes('design')))
      );
    });

    if (matchedUsers.length === 0 && (targetStr.includes('tech') || targetStr.includes('assign'))) {
      matchedUsers = allUsers.filter((u) => {
        const uRole = u.roleId?.name?.toLowerCase() || '';
        return uRole.includes('tech') || uRole.includes('design') || uRole.includes('engineer');
      });
    }
    ...
    if (matchedUsers.length > 0) {
      for (const targetUser of matchedUsers) {
        ...
        }).catch((e) => console.error('Error sending assignment email:', e.message));
      }
    }
  } catch (err: any) {
...
export const updateEnquiry = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!canEditRfq(userInfo || req.user)) {
      return res.status(403).json({
        success: false,
        message: 'Access Denied: Technical Person is only permitted to upload documents and cannot edit RFQ details.',
      });
    }
    ...
    const updated: any = await Enquiry.findByIdAndUpdate(id, updateData, { new: true });
    ...
    if (body.assignedTo && body.assignedTo !== existing.assignedTo) {
      createAssignmentNotification(updated, body.assignedTo, req.user?.email || 'User').catch(() => {});
    }

    return res.json({ success: true, message: 'Enquiry updated successfully', data: { ...updated.toObject(), id: updated._id.toString() } });
  } catch (error: any) {
    return res.status(400).json({ success: false, message: error.message });
  }
};

export const sendForReview = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const id = req.params.id;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Invalid enquiry ID' });
    ...
    const updated: any = await Enquiry.findByIdAndUpdate(id, { status: 'Under review' }, { new: true });
    ...
    return res.json({ success: true, message: 'Enquiry status updated to Under review', data: { ...updated.toObject(), id: updated._id.toString() } });
  } catch (error: any) {
    return res.status(400).json({ success: false, message: error.message });
  }
};

export const verifyReview = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const id = req.params.id;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Invalid enquiry ID' });
    ...
    if (!canReviewRfq(userInfo || req.user)) {
      return res.status(403).json({
        success: false,
        message: 'Access Denied: You do not have permission to review & verify RFQs.',
      });
    }
    ...
    const remarksNote = req.body.remarks ? ` [Verification Note: ${req.body.remarks}]` : '';

    const updated: any = await Enquiry.findByIdAndUpdate(id, {
      status: 'Verified',
      verifiedBy: reviewerName,
      verifiedAt: getTodayIso(),
      remarks: existing.remarks ? `${existing.remarks}${remarksNote}` : (req.body.remarks || ''),
    }, { new: true });
    ...
    await logActivity({
      ...
      details: { enquiryId: id, rfqId: updated.rfqId, verifiedBy: reviewerName },
    });

    return res.json({
      success: true,
      message: 'Review verified & recommended successfully. Status set to Verified.',
...
export const approveReview = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!canFinalApproveRfq(userInfo || req.user)) {
      return res.status(403).json({
        success: false,
        message: 'Access Denied: Only Admin can grant Final Approval for offer dispatch to client. Other reviewers can verify and recommend.',
      });
    }
    ...
    const finalOfferNo = req.body.offerNo || existing.offerNo || existing.clientRefNo || '';
    const finalOfferDate = req.body.offerDate || existing.offerDate || getTodayIso();
    ...
    await logActivity({
      ...
      details: { enquiryId: id, rfqId: updated.rfqId, offerNo: finalOfferNo, approvedBy: approverName },
    });

    return res.json({ success: true, message: 'Final approval granted successfully. Status set to Approved.', data: { ...updated.toObject(), id: updated._id.toString() } });
  } catch (error: any) {
    return res.status(400).json({ success: false, message: error.message });
  }
};
...
export const uploadAttachment = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const isOfferDoc = kind === 'offer' || kind === 'offer_documents' || String(kind).toLowerCase().includes('offer');
    ...
    const extracted: ExtractedOfferMetadata = isOfferDoc
      ...
      : { enquiryNo: '', offerNo: '', offerDate: '', clientName: '', projectName: '' };

    // 2. Intelligent RFQ Resolution by Enquiry No / Client Name
    let targetEnquiryId = requestedId;
    let matchedEnquiry = isOfferDoc && (extracted.enquiryNo || extracted.offerNo)
    ...
    mirrorAttachmentToDrive(targetEnquiryId, file.originalname, file.mimetype, file.buffer, kind).catch((driveErr) => {
    ...
    const cleanAtt = { ...attachment.toObject(), id: attachment._id.toString(), data: undefined };
...
export const autoMapOfferDocApi = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const file = req.file || (req as any).files?.[0];
    if (!file || !file.buffer) {
      return res.status(400).json({ success: false, message: 'No offer document uploaded' });
    }

    const extracted = await extractOfferDetailsFromDoc(file.buffer, file.originalname);
    ...
    mirrorAttachmentToDrive(targetEnquiryId, file.originalname, file.mimetype, file.buffer, 'offer').catch(() => {});
...
export const getAttachmentFile = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const attId = req.params.attachmentId;
    if (!attId) {
      return res.status(400).json({ success: false, message: 'Invalid attachment ID' });
    ...
    return res.status(404).json({ success: false, message: 'Attachment file content empty' });
  } catch (error: any) {
    return res.status(500).json({ success: false, message: error.message });
  }
};

export const inlineUpdateField = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const id = req.params.id;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Invalid enquiry ID' });
    ...
    if (!allowed.includes(field)) {
      return res.status(400).json({ success: false, message: `Field ${field} cannot be updated inline` });
    }
    ...
    return res.json({ success: true, data: { ...updated.toObject(), id: updated._id.toString() } });
  } catch (error: any) {
    return res.status(400).json({ success: false, message: error.message });
  }
};

export const deleteEnquiry = async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (req.user && !isAdmin(req.user)) {
      return res.status(403).json({ success: false, message: 'Admin privileges required to delete RFQ records.' });
    }
...
export const bulkDeleteEnquiries = async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (req.user && !isAdmin(req.user)) {
      return res.status(403).json({ success: false, message: 'Admin privileges required to bulk delete RFQ records.' });
    }
    ...
    return res.json({ success: true, count: result.deletedCount, message: `Deleted ${result.deletedCount} enquiry(s)` });
  } catch (error: any) {
    return res.status(400).json({ success: false, message: error.message });
  }
};
...
export const importExcel = async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (req.user && !isAdmin(req.user)) {
      return res.status(403).json({ success: false, message: 'Admin privileges required to import Excel data.' });
    }
    ...
    for (const row of rows) {
      const companyName = String(row['Company Name'] || row['Company'] || row['company_name'] || '').trim();
      const itemDescription = String(row['Item Description'] || row['Item'] || row['item_description'] || '').trim();
      ...
      await Enquiry.create({
        ...
        companyName,
        itemDescription,
        dateReceived: String(row['Date Received'] || row['date_received'] || getTodayIso()),
        receivedOn: parseIsoDate(String(row['Date Received'] || row['date_received'] || getTodayIso())),
        contactPerson: String(row['Contact Person'] || row['contact_person'] || ''),
        ...
        offerDate: parseIsoDate(String(row['Offer Date'] || row['offer_date'] || '')),
        remarks: String(row['Remarks'] || row['remarks'] || ''),
      });
      createdCount++;
    }

    return res.json({ success: true, message: `Successfully imported ${createdCount} enquiries from Excel.` });
  } catch (error: any) {
    return res.status(500).json({ success: false, message: error.message });
  }
};
...
export const recordOfferApi = async (req: any, res: Response) => {
  try {
    ...
    if (!enquiry) {
      return res.status(404).json({ ok: false, error: 'No enquiry matched enquiry_id / rfq_id' });
    }

    const finalOfferDate = parseIsoDate(offer_date) || getTodayIso();
    const already = enquiry.offerNo === finalOfferNo;

    const updated: any = await Enquiry.findByIdAndUpdate(enquiry._id, {
      offerNo: finalOfferNo,
      offerDate: finalOfferDate,
      status: enquiry.status === 'Open' || enquiry.status === 'Under review' ? 'Offer Sent' : enquiry.status,
    }, { new: true });
...
export const automationCallbackApi = async (req: any, res: Response) => {
  try {
    ...
    const offer_no = (body.offer_no || body.offerNo || body.offer_number || '').trim();
    const offer_date = body.offer_date || body.offerDate;
    const costingText = (body.costing || body.costingDetails || '').trim();
    const technicalText = (body.technical || body.technicalCalculation || body.remarks || '').trim();
    ...
    } else if (offer_no && (enquiry.status === 'Open' || enquiry.status === 'Under review')) {
      updateData.status = 'Offer Sent';
    } else if (costingText && enquiry.status === 'Open') {
      updateData.status = 'Under review';
    }

    const updated: any = await Enquiry.findByIdAndUpdate(enquiry._id, updateData, { new: true });
    ...
    if (Array.isArray(req.files) && req.files.length > 0) {
      for (const file of req.files) {
        ...
        mirrorAttachmentToDrive(enquiry._id.toString(), fn, file.mimetype || 'application/octet-stream', file.buffer, kind).catch(() => {});
        attachedFiles.push({ id: att._id.toString(), filename: att.filename, kind });
      }
    }
...
export const getAutomationUrlApi = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const id = req.params.id;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Invalid enquiry ID' });
    ...
    const baseUrl = (process.env.AUTOMATION_URL || process.env.NEXT_PUBLIC_AUTOMATION_URL || 'https://automation.encon.co.in').trim();
...
export const getDirectory = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const enquiryAssignees: any[] = await Enquiry.find().select('assignedTo salesResponsibility technical').lean();
...
export const saveDirectory = async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (req.user && !isAdmin(req.user)) {
      return res.status(403).json({ success: false, message: 'Admin privileges required to update assignee directory.' });
    }
    ...
    for (const item of items) {
        ...
        for (const e of allEnquiries) {
          ...
          fields.forEach((field) => {
            ...
            if (val) {
              ...
              if (tokens.some((t: string) => t.toLowerCase() === orig.toLowerCase())) {
                updateObj[field] = tokens
                  .map((t: string) => (t.toLowerCase() === orig.toLowerCase() ? name : t))
                  .join(' / ');
                modified = true;
              }
            }
      ...
      await AssigneeEmail.findOneAndUpdate({ name }, { name, email }, { upsert: true, new: true });
    }
...
export const syncInboxApi = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    return res.json({
      success: true,
      message: `Inbox sync completed. Created ${createdCount} new enquiry record(s).`,
      stats,
    });
  } catch (error: any) {
...
export const getInboxStatusApi = async (req: AuthenticatedRequest, res: Response) => {
  try {
    return res.json({
      success: true,
...
export const openDriveFolderApi = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const id = req.params.id;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Invalid enquiry ID' });
...
export const syncDriveFolderApi = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const id = req.params.id;
    if (!id) {
      return res.status(400).json({ success: false, message: 'Invalid enquiry ID' });
    ...
    for (const att of attachments) {
      if (att.data && att.data.length > 0) {
        await mirrorAttachmentToDrive(id, att.filename, att.contentType, att.data, att.kind);
        synced++;
      }
    }
